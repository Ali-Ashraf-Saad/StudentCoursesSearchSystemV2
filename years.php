...
if (is_dir($dataRoot)) {
    foreach (scandir($dataRoot) ?: [] as $entry) {
        if (preg_match('/^\d{4}_\d{4}_[12]$/', $entry) && is_dir($dataRoot . '/' . $entry)) {
            $years[] = $entry;
        }
    }
}
...
usort($years, function ($a, $b) {
    preg_match('/^(\d{4})_(\d{4})_([12])$/', $a, $ma);
    preg_match('/^(\d{4})_(\d{4})_([12])$/', $b, $mb);
...
$yearOptions = array_map(function ($year) {
    if (preg_match('/^(\d{4})_(\d{4})_([12])$/', $year, $match)) {
        return [
            'key' => $year,
            'label' => $match[1] . '/' . $match[2] . ' — الترم ' . ($match[3] === '1' ? 'الأول' : 'الثاني')
        ];
    }
}, $years);
